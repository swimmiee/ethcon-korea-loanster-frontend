    ...
    <div className="flex flex-col divide-y">
      ...
      {tokenBalances &&
        tokenList.map((token, i) => (
          <div key={i} className="hover:bg-neutral-50 place-items-center py-3 grid grid-cols-[1fr_1.5fr_2fr_32px] gap-2">
            <div className="w-full flex gap-2 items-center">
              <TokenIcon size="xl" token={token} />
              <p className="text-t-lg">{token.symbol}</p>
            </div>
            <p className="text-t-md text-center">
              {toFixedCond(tokenBalances.find((t) => t.address === token.address)?.amount, 2)}
            </p>
            <p className="text-t-md text-center">
